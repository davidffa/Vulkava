...
class Node {
    ...
    constructor(vulkava, options) {
        ...
        this.packetQueue = [];
        this.pool = new undici_1.Pool(`http${this.options.secure ? 's' : ''}://${this.options.hostname}:${this.options.port}`);
        this.ws = null;
    }
    get totalPenalties() {
    ...
    getAllRecords(guildId) {
        if (!guildId || typeof guildId !== 'string')
            throw new TypeError('guildId must be a non-empty string');
        return this.request('GET', `records/${guildId}`);
    }
    /**
