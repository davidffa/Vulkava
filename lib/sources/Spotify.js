...
class Spotify {
    ...
    token;
    renewDate;
    constructor(vulkava, clientId, clientSecret) {
    ...
    buildTrack({ name, artists, external_urls: { spotify }, duration_ms }) {
        const artistNames = artists.map(({ name }) => name).join(', ');
        return new UnresolvedTrack_1.default(this.vulkava, name, artistNames, duration_ms, spotify, 'spotify');
    }
