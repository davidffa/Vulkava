...
class Spotify {
    ...
    buildTrack({ name, artists, external_urls: { spotify }, external_ids: { isrc }, duration_ms }) {
        const artistNames = artists.map(({ name }) => name).join(', ');
        return new UnresolvedTrack_1.default(this.vulkava, name, artistNames, duration_ms, spotify, 'spotify', isrc);
    }
