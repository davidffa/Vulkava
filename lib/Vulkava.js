...
class Vulkava extends events_1.EventEmitter {
    ...
    sendWS;
    static APPLE_MUSIC_REGEX = /^(?:https?:\/\/|)?(?:music\.)?apple\.com\/([a-z]{2})\/(album|playlist|artist|music-video)\/[^/]+\/([^/?]+)(?:\?i=(\d+))?/;
    static SPOTIFY_REGEX = /^(?:https?:\/\/(?:open\.)?spotify\.com|spotify)[/:](track|album|playlist|artist)[/:]([a-zA-Z0-9]+)/;
    static DEEZER_REGEX = /^(?:https?:\/\/|)?(?:www\.)?deezer\.com\/(?:\w{2}\/)?(track|album|playlist)\/(\d+)/;
    // guildId <-> Player
    players;
    lastNodeSorting;
    ...
    constructor(options) {
        ...
        if (options.disabledSources) {
            if (!options.disabledSources.includes('APPLE_MUSIC'))
                this.appleMusic = new AppleMusic_1.default(this);
    ...
    async loadFromAppleMusic(query) {
        if (!this.appleMusic)
            return null;
